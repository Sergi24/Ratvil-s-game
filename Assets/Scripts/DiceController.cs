...
public class DiceController : MonoBehaviour
{
    ...
    public void RollDice()
    {
        ...
        StartCoroutine(RolledDice());
        // Invoke("DownForce", 2f);
    }

    public IEnumerator RolledDice()
    {
        while (!gameObject.GetComponent<Rigidbody>().IsSleeping())
        {
            yield return new WaitForSeconds(1f);
        }
        GameObject.Find("Main Camera").GetComponent<CameraController>().SetPlayersTurn();

        diceNumber = SetDiceSides;
    ...
    private int SetDiceSides
    {
        get
        {
            if (CheckRotation(90, -1, -1))
            {
                if (gameObject.name.Equals("Dice"))
                {
                    return 2;
                }
                else if (gameObject.name.Equals("Dice13"))
                {
                    return 2;
                }
                else
    ...
    private bool CheckRotation(int x, int y, int z)
    {
        if (y == -1 && z == -1)
        {
            if (Math.Abs(transform.eulerAngles.x - x) < 1)
            {
                return true;
            }
            else
        ...
        {
            if (Math.Abs(transform.eulerAngles.y - y) < 1 && Math.Abs(transform.eulerAngles.z - z) < 1)
            {
                return true;
            }
            else
        ...
        {
            if (Math.Abs(transform.eulerAngles.x - x) < 1 && Math.Abs(transform.eulerAngles.z - z) < 1)
            {
                return true;
            }
            else
    ...
    {
        GameObject.Find("Game").GetComponent<Game>().KillDiceSides();
